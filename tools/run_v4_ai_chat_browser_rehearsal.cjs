...
const fixture = `<!doctype html><html lang="zh-CN"><head><link rel="stylesheet" href="/chat.css"></head><body>
...
async function main() {
  ...
  try {
    ...
    // B: real dispatch payload remains auto-routed and avoids model controls.
    ...
    // C: an explicit retry reuses one receipt ID and does not duplicate the user turn.
    ...
    // D: task result opens the existing Work owner through its actual task function.
    ...
    // E: stale results cannot become visible after leaving the Owner Surface.
    await sidebar('chat');
    ...
    // F: Return Legacy restores focus to a visible legacy heading.
    await sidebar('chat');
    ...
    // G: the narrow surface retains its labelled compositor without page overflow.
    ...
    process.stdout.write(JSON.stringify({
      scenarios: ['A:frontstage-not-overview', 'B:auto-dispatch-contract', 'C:retry-stable-receipt', 'D:task-handoff', 'E:stale-response-cancelled', 'F:return-legacy-focus', 'G:narrow-labelled-composer'],
