    ...
    function ensureGroupNaturalGuardFields() {
      ...
      target.innerHTML = '<legend>自然参与与自动续聊 Gate</legend><label>突发窗口（秒）<input id="groupBurstWindowSeconds" type="number" min="5" max="300" value="12"></label><label>窗口消息上限<input id="groupBurstMaxMessages" type="number" min="2" max="30" value="6"></label><label>每日主动回复上限<input id="groupDailyReplyBudget" type="number" min="0" max="200" value="20"></label><label>安静间隔（秒）<input id="groupQuietGapSeconds" type="number" min="0" max="120" value="8" aria-describedby="groupContinuationHelp"></label><label>无 @ 续聊窗口（秒）<input id="groupContinuationWindowSeconds" type="number" min="15" max="600" value="120" aria-describedby="groupContinuationHelp"></label><label>无 @ 连续接话上限<input id="groupMaxAutoContinuations" type="number" min="1" max="3" value="2" aria-describedby="groupContinuationHelp"></label><span id="groupContinuationHelp" class="compact-note">连续活跃话题不会无限等待安静：参与强度同时决定有界候选节奏和模型证据门槛；普通群聊仍受预算、冷却、密度与 Delivery confirmed ACK 约束。</span><label class="checkbox-line"><input id="groupNaturalGuardEnabled" type="checkbox">启用全局自然参与</label><button class="secondary" type="button" data-group-natural-cutover>应用总开关</button>';
    }
