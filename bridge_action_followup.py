...
_VERIFY_HINTS = (
    "验证", "检查一下", "检查下", "确认生效", "确认一下", "核对", "验收", "看看是否生效",
)
...
def verify_recent_action(
    connect: Callable[[], sqlite3.Connection],
    *,
    message: str,
    recent_actions: list[dict] | None,
) -> dict | None:
    """Verify a recent action using the owning domain, never the language model."""
    ...
    if action_type != "qq_group_policy_clone":
        # The continuity receipt is still useful evidence for non-QQ actions;
        # domain-specific verifiers can be added without changing routing.
        return {
            "ok": True,
            "dispatch": "action_followup_status",
            "reply": "已识别为上一动作的后续请求，并找到服务端完成回执；当前领域的专用验证器尚未接入，本轮没有冒充执行新的验证动作。",
            "action_receipts": [{
                "action_type": "action.followup.verify",
                "status": "failed",
                "facts": {"bound_action_type": action_type, "source": "continuity_receipt", "reason": "domain_verifier_unavailable"},
            }],
    ...
    if not target_group_id or not source_group_id:
        return {
            "ok": True,
            "dispatch": "action_followup_failed",
            "reply": "上一动作有完成回执，但缺少策略对齐目标，无法安全验收。",
            "action_receipts": [{"action_type": "action.followup.verify", "status": "failed", "facts": {"reason": "target_missing"}}],
        }
    ...
    if not target or not source:
        return {
            "ok": True,
            "dispatch": "action_followup_failed",
            "reply": "上一动作的群策略对象缺失，验收未通过；没有把缺失对象说成已生效。",
            "action_receipts": [{"action_type": "action.followup.verify", "status": "failed", "facts": {"reason": "policy_missing"}}],
        }
    ...
    return {
        ...
        "reply": (
            "已按上一动作的服务端回执重新核对，目标策略与模板策略一致，配置对齐验收通过。"
            if ok else
            "已按上一动作的服务端回执重新核对，但仍有配置差异，验收未通过。"
        ),
        "action_receipts": [{
            "action_type": "action.followup.verify",
...
def dispatch_action_followup(
    connect: Callable[[], sqlite3.Connection],
    continuity_kernel: object,
    *,
    user_id: str,
    source: str,
    trace_id: str,
    ...
    recent = continuity_kernel.recent_action_context({
        "user_id": user_id,
        "source": source,
        "trace_id": trace_id,
        "message": message,
    ...
    result.update({
        "mode": "work",
        "intent": "ops",
        "source": "action_followup_router",
        "recent_action_context": recent[:3],
    })
    return result


def dispatch_action_followup_for_turn(
    connect: Callable[[], sqlite3.Connection],
    continuity_kernel: object,
    *,
    user_id: str,
    ...
    return dispatch_action_followup(
        connect,
        continuity_kernel,
        user_id=user_id,
        source=source,
...
def dispatch_action_followup_context(
    connect: Callable[[], sqlite3.Connection],
    continuity_kernel: object,
    ...
    return dispatch_action_followup(
        ...
        inbound_context=context.get("inbound_context") if isinstance(context.get("inbound_context"), Mapping) else {},
        delivery_recipient_id=str(context.get("delivery_recipient_id") or ""),
    )
